...
python scripts/trade.py positions                                            # 当前持仓（注：双向持仓模式下可能显示不全，用 v3/positionRisk 更准）
python scripts/trade.py orders [--symbol <SYMBOL>]                           # 挂单查询
python scripts/trade.py order <SYMBOL> BUY/SELL <QTY> [--leverage N] [--type MARKET|LIMIT] [--price P]  # 下单
python scripts/trade.py cancel <SYMBOL> ORDER_ID                             # 撤单
```

### 持仓详情（推荐，双向持仓模式下更准确）
```bash
python -c "
import asyncio, httpx, hmac, hashlib, time
from urllib.parse import urlencode
from config.settings import settings
async def main():
    params = {'timestamp': int(time.time()*1000)}
    qs = urlencode(params)
    sig = hmac.new(settings.binance_api_secret.encode(), qs.encode(), hashlib.sha256).hexdigest()
    params['signature'] = sig
    async with httpx.AsyncClient(base_url=settings.binance_futures_base, headers={'X-MBX-APIKEY': settings.binance_api_key}, timeout=15) as c:
        resp = await c.get('/fapi/v3/positionRisk', params=params)
        for p in resp.json():
            amt = float(p.get('positionAmt',0))
            if amt != 0:
                symbol, side = p['symbol'], p['positionSide']
                entry, mark = float(p['entryPrice']), float(p['markPrice'])
                pnl, margin = float(p['unRealizedProfit']), float(p['initialMargin'])
                pnl_pct = (pnl/margin*100) if margin > 0 else 0
                print(f'{symbol} {side} entry={entry:.7f} mark={mark:.7f} PnL={pnl:+.4f}U ({pnl_pct:+.1f}%)')
asyncio.run(main())
"
```

### 止损单（Algo Order，Binance 2025-12 迁移后必须用这个）
```python
# 设止损
from src.execution.binance_client import BinanceTradingClient
client = BinanceTradingClient('futures_um')
await client.place_algo_order('<SYMBOL>', 'SELL', 'STOP_MARKET', trigger_price=<PRICE>, position_side='LONG', close_position=True)

# 查看当前止损单
# GET /fapi/v1/openAlgoOrders

# 取消止损
await client.cancel_algo_order('<SYMBOL>', <algoId>)
```
